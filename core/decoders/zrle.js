...
export default class ZRLEDecoder {
    ...
    decodeRect(x, y, width, height, sock, display, depth) {
        ...
        if (this._data === true) {
            ...
            let rx = 0, ry = 0, tileNo = 0;
            for (var ty = 1; ty <= totalTilesY; ty++) {
                rx = 0;
                for (var tx = 1; tx <= totalTilesX; tx++) {
                    ...
                    if (subencoding == 0) { // Raw pixel data
                        ...
                        for (var src = 0, dst = 0; src < bytes; src += 3, dst += 4) {
                    ...
                    if (subencoding >= 2 && subencoding <= 16) { // Packed palette
                        ...
                        for (var tileY = 0; tileY < tileHeight; tileY++) {
                            ...
                            for (var tileX = 0, bitIdx = 0, byteIdx = strideWidth * tileY; tileX < tileWidth; tileX++) {
                                switch (bitsPerPixel) {
                                    case 1:
